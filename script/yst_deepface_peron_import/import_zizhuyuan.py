...
con = pymysql.connect("localhost", user="root", passwd="123456",
                      port=3306, db="CorsfaceRepo", charset="utf8")
cur = con.cursor()
...
def createPersonGroup(groups):
    ...
    for group in groups:
        cur.execute(
            "SELECT id FROM person_group where group_name = '%s'" % (group))
        result = cur.fetchall()
        if not len(result):
            cur.execute("INSERT INTO person_group(group_uuid, group_name, alarm_sound_id, remark, created_time) VALUES('%s', '%s', 1, '%s', now())" % (
                str(uuid.uuid1()), group, group))
            con.commit()
...
def createPerson(if_crop):
    ...
    for person_info in persons:
        # print('>>> %s' % person_info)
        count = count + 1
        print(">>> %s in whole person %s, person_info %s" %
              (count, whole_count, person_info))
...
def recognise_img(person_info, if_crop, img_type):
    ...
    if if_crop == 1:
        base64_image, sex = cropface(
            base64_image, DEEPFACE_URL, DEEPFACE_APP_KEY)
        if sex == 1:
            sex = 1
        else:
            # Corsface 中女为1
    ...
    cur.execute(
        "SELECT person_id FROM person_info where id_card = '%s'" % (id_card))
    ...
    cur.execute("INSERT INTO person_image(person_id, image_id, face_image, isdeleted, created_time) values('%s', '%s', '%s', 0, now())" % (
        person_id, str(uuid.uuid1()), image_path))
    con.commit()
